...
The `home-routes` handler in the `clojure-app.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-app.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clojure-app.handler` namespace:
...
Request middleware functions are located under the `clojure-app.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-app.dev-middleware` namespace found in
